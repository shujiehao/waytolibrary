 ...
 */
public class WaytoMapLayer extends TiledServiceLayer {
    private final String TAG = getClass().getSimpleName();

    /*离线地图默认缓存路径*/
    private String defult_map_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Wayto/map/";

    private Context context;
    private int mWaytoMapLayerType;
    private int dpi = 96;
    ...
    public WaytoMapLayer(int paramInt, Context context) {
        super(true);
        this.context = context;
        this.mWaytoMapLayerType = paramInt;
        initLayer();
    ...
    protected byte[] getTile(int level, int col, int row) throws Exception {
        ...
        if (level < 11) {
            bytes = readBaseMap(new File(defult_map_dir + "BaseMap.db"), mWaytoMapLayerType, level, col, row);
        } else {
            //bytes = readSDDB(new File(defult_map_dir + "SZS-ShenZhenShi.db"), mWaytoMapLayerType, level, col, row);
                ...
                for (File file : files) {
                    ...
                    if (fileName.contains("-")) {
                        ...
                        if (minCol < col && col < maxCol && minRow < row && row < maxRow) {
                            bytes = readBaseMap(file, mWaytoMapLayerType, level, col, row);
                            if (bytes != null)
                                break;
                        }
                    }
        ...
        //2、若没有查询APP内部自动缓存数据
        if (bytes == null) {
            bytes = queryTileToLocal(mWaytoMapLayerType, level, col, row);
        }
        ...
        if (bytes == null) {
            ...
            //4、缓存APP内部
            saveTileToLocal(mWaytoMapLayerType, level, col, row, bytes);
        }
